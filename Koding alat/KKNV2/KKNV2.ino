...
#include "DHT.h"
#include <DHT_U.h>
#define pompa1  13 //Relay untuk Pompa
...
Time tt;
int jam, menit, detik;
int set_jam1 = 16;     //Set alarm jam Pagi
int set_menit1 = 34, set_menit1b = 35;   //Set alarm menit Pagi
...
// Delay dalam Milis untuk pompa
unsigned long interval = 60000; // the time we need to wait
unsigned long previousMillis = 0; // millis() returns an unsigned long.
bool pompaState = false; // state variable for the Pompa
...
void setup() {
  ...
  pinMode(8, INPUT);
  pinMode(9, INPUT);
  digitalWrite(pompa1, pompaState);
...
void siram1() {
  if (set_jam1 == jam) {             //Jika jam sama dengan alarm, maka
    if (set_menit1 == menit || set_menit1b == menit ) {       //Jika menit sama dengan alarm, maka
      spray();
...
void spray()
{
  ...
  while ((currentMillis - previousMillis) < interval) {
    pompaState = true; // "toggles" the state
    digitalWrite(pompa1, pompaState); // sets the LED based on ledState
  ...
  /*
    if ((currentMillis - previousMillis) > interval) {
    pompaState = false; // "toggles" the state
    digitalWrite(pompa1, pompaState); // sets the LED based on
    // save the "current" time
    previousMillis = millis();
    }
  */
